...
\item \textbf{Wind turbin cut} All events reconstructing to within $5^\circ$ of the wind turbine's azimuth are rejected.
...
The final analysis cuts and the result of their application to the analysis data sample are detailed in \TableRef{tab:Analysis:Cut-Flow}.
...
\begin{table}
  \begin{center}
    \begin{tabular}{ c || c | c | c }
      ...
      CW & $9.66 \times 10^5$ & $1.66 \times 10^3$ & $1.07 \times 10^7$\\
      CWMinProb & $5.76 \times 10^5$ & 710 & $7.27 \times 10^5$\\
      ...
      CW & $4.21 \times 10^5$ & 456 & $9.67 \times 10^6$\\
      CWMinProb & $2.97 \times 10^5$ & 331 & $2.44 \times 10^5$\\
...
Something about the cut efficiencies
...
Estimating the expected number of background events falls into two chategories: thermal backgrounds and anthroprogenic backgrounds. The thermal background estimation is based upon the distribution of powherence values for minimum bias events with all other cuts applied. This sample of events is the most pure sample of thermal noise events and can be used to extrapolate the number of expected thermal events passing the powherence cut having taken into account the increase in statistics between the minimum bias and analysis sample of events. Figure \FigureRef{fig:Analysis:Background-Estimation:Thermal-Background} shows the distributions and fits to data for VPol and HPol events. The resulting expected number of VPol (HPol) thermal events is $0.03 \pm 0.02$ ($0.15 \pm 0.07$), with the combined expectation of $0.18 \pm 0.07$ events in the full data sample.
...
For the case of anthroprogenic signals it is much more difficult to ascertain the expected background levels since these events are from an unidentified mixture of sources, which are not necessarily expected to follow well described distributions. An estimate was made by considering the events that failed the CW minimum probability cut as the last cut applied. This selection of events should be composed of only anthroprogenic signals that contain some level of CW contamination which are distributed up to the cut threshold of -400 described in \SectionRef{sec:Analysis:CWRemoval}. The tail adjacent to the cut value was fitted with a gaussian which is shown in \FigureRef{fig:Analysis:Background-Estimation:CW-Background} which is sensitive to both the binning and range of values that are considered in the fit. A conservative approach was taken in estimating the expected background levels and the fit shown results in a lower number of expected background events than other estimates made. The expected number of background events is $0.19 \pm 0.28$.
